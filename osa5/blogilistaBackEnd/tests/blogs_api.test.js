...
describe('default tests', () => {
  ...
  test('just added blog is found from the database', async () => {
    const loginInfo = await loginAsUser("placeholder", "1234")
      ...
      const addedBlog = await api.post('/api/blogs').set('Authorization', `Bearer ${loginInfo.token}`).send(newBlog)
  ...
  test('blog created with empty likes is assinged 0 likes when added to database', async () => {
    const loginInfo = await loginAsUser("placeholder", "1234")
      ...
      const savedBlog = await api.post('/api/blogs').set('Authorization', `Bearer ${loginInfo.token}`).send(newBlog)
  ...
  test('if new blog title is empty return 400', async () => {
    const loginInfo = await loginAsUser("placeholder", "1234")
      ...
      await api.post('/api/blogs').set('Authorization', `Bearer ${loginInfo.token}`).send(newBlog).expect(400)
  ...
  test('deleted blog is removed from the database', async () => {
    ...
    await api.delete(`/api/blogs/${blogToBeRemoved.id}`).set('Authorization', `Bearer ${loginInfo.token}`)
  ...
  test('changing a blog changes values correctly', async () => {
    const loginInfo = await loginAsUser("placeholder", "1234")
    const blogToBeChanged = await Blog.findOne({title: "This blog is owned by username placeholder"})
   
    const newLikes = blogToBeChanged.likes + 1
    
    const response = await api.put(`/api/blogs/${blogToBeChanged.id}`).set('Authorization', `Bearer ${loginInfo.token}`).send({likes: newLikes})
  ...
  test('changing a missing blog returns status code 404 missing', async () => {
    ...
    await api.delete(`/api/blogs/${blogId}`).set('Authorization', `Bearer ${loginInfo.token}`).expect(204)
    await api.put(`/api/blogs/${blogId}`).set('Authorization', `Bearer ${loginInfo.token}`).send(blogToBeChanged).expect(404)
  })
  ...
  test('creating a blog when logged in as a user assings the user to the blog correctly', async () => {
   ...
   const savedBlog =  await api.post('/api/blogs').set('Authorization', `Bearer ${loginInfo.token}`).send(newBlog).expect(201)
  ...
  test('creating a blog when logged in as a user assings the blog to the user correctly', async () => {
   ...
   const savedBlogReq =  await api.post('/api/blogs').set('Authorization', `Bearer ${loginInfo.token}`).send(newBlog).expect(201)
...
describe('blog permission tests', () => {
  test('creating a blog when logged in as a user succeeds', async () => {
    ...
    await api.post('/api/blogs').set('Authorization', `Bearer ${loginInfo.token}`).send(newBlog).expect(201)
  ...
  test('creating a blog when not logged in as a user fails with errorcode 401 and doesnt create a blog', async () => {
   ...
   const serverResponse =  await api.post('/api/blogs').send(newBlog).expect(401)
   const blog = await Blog.findOne({title: "new blog"})
  ...
  test('removing a blog when logged in as a user that created the blog succeeds', async () => {
    const loginInfo = await loginAsUser("placeholder", "1234")
   const blogToBeRemoved = await Blog.findOne({title: "This blog is owned by username placeholder"})
   const removedBlog =  await api.delete(`/api/blogs/${blogToBeRemoved.id}`).set('Authorization', `Bearer ${loginInfo.token}`).expect(204)
   
   const blog = await Blog.findOne({title: "This blog is owned by username placeholder"})
   expect(blog).toBe(null)
  })
  ...
  test('removing a blog when not logged in as a user that created the blog fails with errorcode 401', async () => {
   ...
   const blogToBeRemoved = await Blog.findOne({title: "This blog is owned by username placeholder"})
   const removedBlog =  await api.delete(`/api/blogs/${blogToBeRemoved.id}`).set('Authorization', `Bearer ${loginInfo.token}`).expect(401)
...
beforeEach(async () => {
    ...
    await api.post('/api/blogs').set('Authorization', `Bearer ${authToken.token}`).send(blogs[0])
   
   
    const secondAuthToken = await loginAsUser("second placeholder", "1234")
    await api.post('/api/blogs').set('Authorization', `Bearer ${secondAuthToken.token}`).send(blogs[1])
