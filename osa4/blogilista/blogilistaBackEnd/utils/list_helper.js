...
const favoriteBlog = (blogs) => {
  ...
  {
    return {title: sortedBlogs[0].title, author: sortedBlogs[0].author, likes: sortedBlogs[0].likes}
  }
  else{
    return undefined
  }
...
const mostLikes = (blogs) => {
  ...
  blogs.forEach(blog => {
    if(!authorBlogLikes.has(blog.author)){
      ...
      authorBlogLikes.set(blog.author, authorBlogLikes.get(blog.author) + blog.likes)
    }
  })
